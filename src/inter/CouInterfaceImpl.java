...
public class CouInterfaceImpl implements CouInterface {
	...
	public ArrayList<CouseDAO> selectCouseAll(Connection conn) {
		ArrayList<CouseDAO> CouseDAOList = null;
		try {
			...
			CouseDAOList = new ArrayList<CouseDAO>();
			while (rs.next()) {
				int no = rs.getInt("NO");
				...
				CouseDAO couseDAO = new CouseDAO(no, num, id, pwd);
				CouseDAOList.add(couseDAO);
			}
	...
	// 수강 1개만 불러오기
	public SubjectDAO selectCosOne(Connection conn, int i_no) {
